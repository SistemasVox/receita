                    ...
                    if check_table_existence; then
                        ...
                        for csv_file in "$csv_path"/*; do
                            ...
                            if [ -e "$csv_file" ]; then
                                ...
                                echo -e
                                if mysqlimport --host="$db_host" --user="$db_user" --password="$db_password" --ignore-lines=1 --fields-terminated-by="$separator" --fields-enclosed-by="$delimiter" --verbose --local "$db_name" "$csv_file" >/dev/null 2>&1; then
