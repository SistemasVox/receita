...
# Exibir todos os links dos arquivos zip relacionados aos Cnaes
echo -e "\nLinks dos arquivos zip relacionados aos Cnaes:"
curl -s "$url" | awk -F 'href="' '/Cnaes/ && /zip"/ {print $2}' | cut -d '"' -f 1
echo -e "\n"

# Baixar cada arquivo zip
downloaded_files=0
for file in $(curl -s "$url" | awk -F 'href="' '/Cnaes/ && /zip"/ {print $2}' | cut -d '"' -f 1)
do
...
file="$csv_dir/cnae.csv"
# Importar os dados para a tabela
if mysql --host="$db_host" --user="$db_user" --password="$db_password" --local-infile=1 --max_allowed_packet=1G -e "USE $db_name; DROP TABLE IF EXISTS cnae; CREATE TABLE cnae (cnae INT, descricao VARCHAR(255)); LOAD DATA LOCAL INFILE '$file' INTO TABLE cnae FIELDS TERMINATED BY '$separator' ENCLOSED BY '$delimiter' LINES TERMINATED BY '\n';"; then
  # Mensagem de sucesso
