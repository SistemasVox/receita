...
# Exibir todos os links dos arquivos zip relacionados aos Municipios
echo -e "\nLinks dos arquivos zip relacionados aos Municipios:"
curl -s "$url" | awk -F 'href="' '/Municipios/ && /zip"/ {print $2}' | cut -d '"' -f 1
echo -e "\n"

# Baixar cada arquivo zip
downloaded_files=0
for file in $(curl -s "$url" | awk -F 'href="' '/Municipios/ && /zip"/ {print $2}' | cut -d '"' -f 1)
do
			...
			for csv_file in "$csv_dir"/*; do
				if [[ -f "$csv_file" ]]; then
					...
					if [ "$file_extension" != "csv" ]; then
						new_filename="$csv_dir/municipios.csv"
...
file="$csv_dir/municipios.csv"
# Importar os dados para a tabela
if mysql --host="$db_host" --user="$db_user" --password="$db_password" --local-infile=1 --max_allowed_packet=1G -e "USE $db_name; DROP TABLE IF EXISTS municipios; CREATE TABLE municipios (cnae INT, nome VARCHAR(255)); LOAD DATA LOCAL INFILE '$file' INTO TABLE municipios FIELDS TERMINATED BY '$separator' ENCLOSED BY '$delimiter' LINES TERMINATED BY '\n';"; then
  # Mensagem de sucesso
